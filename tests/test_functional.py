"""
Модульні тести, що охоплюють основні функціональні вимоги платформи:

  1. Реєстрація волонтера → автоматична верифікація
  2. Неверифікований координатор → не може працювати
  3. Верифікація організації → каскадно встановлює is_verified для координаторів-членів
  4. Властивість Task.slots_available
  5. Волонтер приймає завдання → статус переходить у IN_PROGRESS
  6. Останній волонтер відмовляється → статус повертається до OPEN
  7. Видалення завдання з призначеними волонтерами → архівується, не видаляється
  8. VolunteerProfile.completed_tasks_count() рахує лише COMPLETED завдання
"""
import pytest
